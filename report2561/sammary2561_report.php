    ...
    <main class="app-content">
        <div class="app-title">
            ...
            <ul class="app-breadcrumb breadcrumb side">
                ...
                <li class="breadcrumb-item active"><a href="#">ปีงบประมาณ 2561</a></li>
            </ul>
        </div>
        ...
        $report2561_count = $report1_count['total'] + $report2_count['total'] + $report3_count['total'];
        $total2561_report_per = ($total_report1_per + $total_report2_per + $total_report3_per) / 3;
        mysqli_close($conn);
        ...
        <div class="col-md-12">
            <div class="tile">
                <h3 class="tile-title">สรุปผลการดำเนินงาน ปีงบประมาณ 2561</h3>
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr>
                            <td>1</td>
                            <td>ภารกิจหลัก</td>
                            <td><?php echo $report1_count['total'] ?></td>
                            <td><?php echo number_format($total_report1_per, 2, '.', '')  ?></td>
                            <?php if ($total_report1_per >= 50) {
                                echo "<td><a href='sammary_report1.php' class='btn btn-success' role='button' aria-pressed='true'>ผ่าน</a></td>";
                            } else {
                                echo "<td><a href='sammary_report1.php' class='btn btn-danger' role='button' aria-pressed='true'>ไม่ผ่าน</a></td>";
                            } ?>
                        </tr>
                        <tr>
                            <td>2</td>
                            <td>สำนักงบประมาณ</td>
                            <td><?php echo $report2_count['total'] ?></td>
                            <td><?php echo number_format($total_report2_per, 2, '.', '') ?></td>
                            <?php if ($total_report2_per >= 50) {
                                echo "<td><a href='sammary_report2.php' class='btn btn-success' role='button' aria-pressed='true'>ผ่าน</a></td>";
                            } else {
                                echo "<td><a href='sammary_report2.php' class='btn btn-danger' role='button' aria-pressed='true'>ไม่ผ่าน</a></td>";
                            } ?>
                        </tr>
                        <tr>
                            <td>3</td>
                            <td>แผนยุทธศาสตร์</td>
                            <td><?php echo $report3_count['total'] ?></td>
                            <td><?php echo number_format($total_report3_per, 2, '.', '') ?></td>
                            <?php if ($total_report3_per >= 50) {
                                echo "<td><a href='sammary_report3.php' class='btn btn-success' role='button' aria-pressed='true'>ผ่าน</a></td>";
                            } else {
                                echo "<td><a href='sammary_report3.php' class='btn btn-danger' role='button' aria-pressed='true'>ไม่ผ่าน</a></td>";
                            } ?>
                        </tr>
                        <tr>
                            <td>N</td>
                            <td>สรุปผลภาพรวมผลดำเนินงาน</td>
                            <td><?php echo $report2561_count ?></td>
                            <td><?php echo number_format($total2561_report_per, 2, '.', '') ?></td>
                            <?php if ($total2561_report_per >= 50) { ?>
