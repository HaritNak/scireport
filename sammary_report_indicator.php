...
<head>
    ...
    <title>รายงานสรุปผลดำเนินงาน 2561</title>
    <meta charset="utf-8">
...
<body class="app sidebar-mini rtl">
    ...
    $sql_select_operation = "SELECT o_name FROM $table_operation_name where o_id=$o_id ";
    ...
    <main class="app-content">
        <div class="app-title">
            ...
            <ul class="app-breadcrumb breadcrumb side">
                ...
                <li class="breadcrumb-item active"><a href="sammary_report.php?year=<?php echo $fiscal_year ?>">ปีงบประมาณ <?php echo $fiscal_year ?></a></li>
            </ul>
        </div>

        <?php

        include_once('process_report.php');
        ?>

        <div class="clearfix"></div>
        <div class="col-md-12">
            <div class="tile">
                <h3 class="tile-title">สรุปผลตัวชี้วัดหลัก <?php echo $row_select_operation['o_name'] ?></h3>
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                        $sql_select_indicator = "SELECT i_id,i_name FROM $table_indicator_name where o_id =  $o_id ";
                        ...
                        if (!$result_select_indicator) {
                            ...
                            while ($row_select_indicator = mysqli_fetch_array($result_select_indicator)) {
                                echo "<tr>";
                                echo "<td>" . $row_select_indicator['i_id'] . "</td>";
                                echo "<td>" . $row_select_indicator['i_name'] . "</td>";

                                echo "<td>" . $total_count[$row_select_indicator['i_id']] . "</td>";

                                echo "<td>" .  number_format((float)$report_per[$row_select_indicator['i_id']], 2, '.', '')   . "</td>";

                                if ($report_per[$row_select_indicator['i_id']] >= 50 && $report_per[$row_select_indicator['i_id']] != 0) {
                                    echo "<td><a href='sammary_report_sub_indicator.php?year= " . $fiscal_year . "&&o_id=" . $o_id . "&&i_id=" .  $row_select_indicator['i_id'] . " ' class='btn btn-success' role='button' aria-pressed='true'>ผ่าน</a></td>";
                                } else if ($report_per[$row_select_indicator['i_id']] < 50 && $report_per[$row_select_indicator['i_id']] != 0) {
                                    echo "<td><a href='sammary_report_sub_indicator.php?year= " . $fiscal_year . "&&o_id=" . $o_id . "&&i_id=" .  $row_select_indicator['i_id'] . " ' class='btn btn-danger' role='button' aria-pressed='true'>ไม่ผ่าน</a></td>";
                                } else {
                                    echo "<td><a href='sammary_report_sub_indicator.php?year= " . $fiscal_year . "&&o_id=" . $o_id . "&&i_id=" .  $row_select_indicator['i_id'] . " ' class='btn btn-warning' role='button' aria-pressed='true'>ไม่ทราบ</a></td>";
                                }
                                echo "</tr>";
                            }
                        }
