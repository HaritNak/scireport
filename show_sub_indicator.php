...
<body class="app sidebar-mini rtl">
        ...
        <ul class="app-menu">
            ...
            <?php if (isset($_SESSION['id'])) { ?>
                ...
                <?php if ($_SESSION['status'] == "ผู้บันทึก" || $_SESSION['status'] == "ผู้รับผิดชอบ") { ?>
                    <li class="treeview"><a class="app-menu__item active" href="#" data-toggle="treeview"><i class="app-menu__icon fa fa-sticky-note-o"></i><span class="app-menu__label">เลือกสูตรตัวชี้วัดย่อย</span><i class="treeview-indicator fa fa-angle-right"></i></a>
                        <ul class="treeview-menu">
                            <!-- sql -->
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-sticky-note-o"></i> สูตรตัวชี้วัดย่อย ปีงบประมาณ<?php echo  $fiscal_year ?></h1>

            </div>
            <ul class="app-breadcrumb breadcrumb side">
                <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
                <li class="breadcrumb-item active"><a href="#">สูตรตัวชี้วัดย่อย ปีงบประมาณ<?php echo  $fiscal_year ?></a></li>
            </ul>
        </div>
        ...
        <div class="col-md-12">
            <div class="tile">
                <h3 class="tile-title">สูตรตัวชี้วัดย่อย ปีงบประมาณ<?php echo  $fiscal_year ?></h3>
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php
                        $sql_select_member = "SELECT s.s_id,s.s_name,q.q_type,q.q_table_1,q.q_column_1,q.q_table_2,q.q_column_2,q.q_full FROM $table_sub_indicator_name s join $table_sql_sub_indicator q on  s.s_id=q.q_id  WHERE s.s_id_record =" . $_SESSION['id'] . " OR s.s_id_respon =" . $_SESSION['id'] . "";
                        $result_select_member = $conn->query($sql_select_member);
                        if (!$result_select_member) {
                        } else {
                            while ($row_select_member = mysqli_fetch_array($result_select_member)) {
                                echo "<tr>";
                                echo "<td>" . $row_select_member['s_id'] . "</td>";
                                echo "<td>" . $row_select_member['s_name'] . "</td>";
                                echo "<td>" . $row_select_member['q_type'] . "</td>";
                                echo "<td>" . $row_select_member['q_table_1'] . "</td>";
                                echo "<td>" . $row_select_member['q_column_1'] . "</td>";
                                echo "<td>" . $row_select_member['q_table_2'] . "</td>";
                                echo "<td>" . $row_select_member['q_column_2'] . "</td>";
                                if ($_SESSION['status'] == "ผู้บันทึก") {
                                    if ($row_select_member['q_full'] != NULL) {
                                        echo "<td><a href='insert_sql_sub_indicator.php?year=" . $fiscal_year . "&&s_id=" . $row_select_member['s_id']  . "' class='btn btn-success' role='button' aria-pressed='true'>เลือก</a></td>";
                                    } else {
                                        echo "<td><a href='insert_sql_sub_indicator.php?year=" . $fiscal_year . "&&s_id=" . $row_select_member['s_id']  . "' class='btn btn-warning' role='button' aria-pressed='true'>เลือก</a></td>";
                                    }
                                }
                                echo "</tr>";
                            }
