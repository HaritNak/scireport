...
<head>
    ...
    <title> เพิ่มตัวชี้วัดหลัก</title>
    <meta charset="utf-8">
...
<body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-plus-square-o"></i> เพิ่มตัวชี้วัดหลัก<?php echo  $row_select_operation['o_name'] . "ปีงบประมาณ" . $fiscal_year    ?></h1>
            ...
            <ul class="app-breadcrumb breadcrumb side">
                ...
                <li class="breadcrumb-item active"><a href="#">เพิ่มตัวชี้วัดหลัก</a></li>
            </ul>
        </div>
        ...
        <div class="row">
            <div class="col-md-8 mx-auto">
                <div class="tile">
                    <h3 class="tile-title">เพิ่มตัวชี้วัดหลัก<?php echo  $row_select_operation['o_name'] . "ปีงบประมาณ" . $fiscal_year  ?></h3>
                    <form action="insert_indicator_action.php" method="POST">
                        <div class="form-group row">
                            <div class="form-group col-md-10">
                                <input type="text" class="form-control" id="i_name" name="i_name" placeholder="ชื่อตัวชี้วัดหลัก" required>
                            </div>
                            <div class="form-group col-md-6">
                                <input type="text" class="form-control" id="i_name_s" name="i_name_s" placeholder="ชื่อตัวชี้วัดหลักสำหรับกราฟ (มากสุดได้แค่ 20 ตัวอักษร)" maxlength="20" required>
                            </div>
                            <div class="form-group col-md-4">
                                <input type="text" class="form-control" id="i_goal" name="i_goal" placeholder="ค่าเป้าหมาย" required>
                    ...
                    <table class="table table-hover table-bordered">
                        <thead>
                            <tr>
                                <th>ลำดับที่</th>
                                <th>ชื่อตัวชี้วัดหลัก</th>
                                <th>ชื่อตัวชี้วัดหลัก(กราฟ)</th>
                                <th>ค่าเป้าหมาย</th>
                                <th>เพิ่มตัวชี้วัดย่อย</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $sql_select_indicator = "SELECT row_number() OVER (ORDER BY i_id) id,i_id,i_name,i_name_s,i_goal FROM $table_indicator_name where o_id =  $o_id ";
                            ...
                            if (!$result_select_indicator) {
                                ...
                                while ($row_select_indicator = mysqli_fetch_array($result_select_indicator)) {
                                    ...
                                    echo "<td>" . $row_select_indicator['i_name_s'] . "</td>";
                                    echo "<td>" . $row_select_indicator['i_goal'] . "</td>";
                                    echo "<td><a href='insert_sub_indicator.php?year= " . $fiscal_year . "&&o_id=" . $o_id . "&&i_id=" .  $row_select_indicator['i_id'] . " ' class='btn btn-success' role='button' aria-pressed='true'>เพิ่ม</a></td>";
                                    echo "</tr>";
                                }
                            }
                            ?>
